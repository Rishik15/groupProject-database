...
INSERT INTO plan_exercise (plan_id, exercise_id, order_in_workout, sets_goal, reps_goal, weight_goal) VALUES
  ...
  (1, 10, 2, 3, 10,  NULL),    -- impossible dips
  (1, 20, 3, 4, 10,  NULL),    -- lever front pulldown
  (1, 21, 4, 4, 10,  NULL),    -- lever seated row
  (1, 18, 5, 3, 12,  NULL),    -- dumbbell reverse spider curl
  (1,  1, 6, 4, 12,  NULL),    -- hack calf raise
  (1,  2, 7, 4, 10,  NULL),    -- sled 45° leg press
  (1,  9, 8, 3,  8,  NULL),    -- kettlebell pistol squat
  -- plan 2: Upper Lower Split
  (2,  5, 1, 3, 10,  NULL),    -- barbell incline bench press
  (2,  3, 2, 3, 12,  NULL),    -- dumbbell front raise
  (2, 14, 3, 3, 12,  NULL),    -- dumbbell one arm upright row
  (2,  2, 4, 3, 12,  NULL),    -- sled 45° leg press
  (2, 28, 5, 3, 15,  NULL),    -- barbell standing calf raise
  (2,  1, 6, 3, 15,  NULL),    -- hack calf raise
  -- plan 3: Full Body Strength
  (3,  5, 1, 4,  6, 135.00),   -- barbell incline bench press
  (3, 21, 2, 4,  6,  95.00),   -- lever seated row
  (3,  2, 3, 4,  8, 180.00),   -- sled 45° leg press
  (3, 11, 4, 3, 10,  65.00),   -- barbell seated overhead triceps extension
  (3, 28, 5, 3, 12,  NULL),    -- barbell standing calf raise
  -- plan 4: Arms & Shoulders
  (4,  8, 1, 4, 10,  NULL),    -- barbell standing close grip curl
  (4,  7, 2, 3, 12,  NULL),    -- dumbbell one arm hammer preacher curl
  (4,  6, 3, 3, 12,  NULL),    -- cable squatting curl
  (4, 18, 4, 3, 15,  NULL),    -- dumbbell reverse spider curl
  (4, 11, 5, 4, 10,  NULL),    -- barbell seated overhead triceps extension
  (4,  3, 6, 3, 15,  NULL),    -- dumbbell front raise
  -- plan 5: Beginner Bodyweight
  (5, 10, 1, 3, 10,  NULL),    -- impossible dips
  (5, 19, 2, 3, 15,  NULL),    -- bent knee lying twist
  (5,  3, 3, 3, 12,  NULL),    -- dumbbell front raise
  -- plan 6: Core & Conditioning
  (6, 19, 1, 3, 20,  NULL),    -- bent knee lying twist
  (6, 30, 2, 3, 15,  NULL),    -- cable seated crunch
  (6, 13, 3, 3, 15,  NULL),    -- weighted side bend
  (6, 26, 4, 3, 10,  NULL),    -- assisted hanging knee raise
  -- plan 7: Cable Machine Intro
  (7,  6, 1, 3, 12,  NULL),    -- cable squatting curl
  (7, 23, 2, 3, 12,  NULL),    -- cable decline fly
  (7, 27, 3, 3, 12,  NULL),    -- cable seated curl
  (7, 30, 4, 3, 15,  NULL),    -- cable seated crunch
  -- plan 8: Kettlebell Fundamentals
  (8,  9, 1, 3,  8,  NULL),    -- kettlebell pistol squat
  (8, 13, 2, 3, 12,  NULL),    -- weighted side bend
  (8, 29, 3, 3, 12,  NULL),    -- weighted hyperextension
  -- plan 9: Strength Foundations
  (9,  5, 1, 5,  5,  NULL),    -- barbell incline bench press
  (9, 21, 2, 5,  5,  NULL),    -- lever seated row
  (9,  2, 3, 5,  5,  NULL),    -- sled 45° leg press
  (9, 28, 4, 3, 10,  NULL),    -- barbell standing calf raise
  -- plan 10: Isolation Arm Builder
  (10,  8, 1, 4, 10,  NULL),   -- barbell standing close grip curl
  (10, 22, 2, 3, 12,  NULL),   -- dumbbell standing concentration curl
  (10, 17, 3, 3, 12,  NULL),   -- dumbbell lying single extension
  (10, 27, 4, 3, 12,  NULL),   -- cable seated curl
  (10, 11, 5, 3, 10,  NULL),   -- barbell seated overhead triceps extension
  -- plan 11: Fat Loss Circuit
  (11, 30, 1, 4, 15,  NULL),   -- cable seated crunch
  (11, 19, 2, 4, 20,  NULL),   -- bent knee lying twist
  (11, 26, 3, 3, 10,  NULL),   -- assisted hanging knee raise
  (11, 10, 4, 3, 12,  NULL),   -- impossible dips
  -- plan 12: Cutting Shred Program
  (12,  6, 1, 4, 15,  NULL),   -- cable squatting curl
  (12, 23, 2, 4, 15,  NULL),   -- cable decline fly
  (12, 30, 3, 4, 20,  NULL),   -- cable seated crunch
  (12, 18, 4, 3, 15,  NULL),   -- dumbbell reverse spider curl
  (12, 13, 5, 3, 15,  NULL),   -- weighted side bend
  -- plan 13: Cardio Endurance Builder
  (13, 19, 1, 3, 25,  NULL),   -- bent knee lying twist
  (13, 26, 2, 3, 12,  NULL),   -- assisted hanging knee raise
  (13, 30, 3, 3, 20,  NULL),   -- cable seated crunch
  (13, 29, 4, 3, 15,  NULL),   -- weighted hyperextension
  -- plan 14: HIIT & Conditioning
  (14, 10, 1, 5, 10,  NULL),   -- impossible dips
  (14, 19, 2, 5, 15,  NULL),   -- bent knee lying twist
  (14,  9, 3, 4,  8,  NULL),   -- kettlebell pistol squat
  (14, 26, 4, 4, 10,  NULL),   -- assisted hanging knee raise
  (14, 30, 5, 4, 15,  NULL);   -- cable seated crunch

-- plans 1-2: system authored public library
-- plans 3-4: coach authored (Sam=3, Taylor=5)
-- plans 5-14: system predefined library for UC 4.2
-- description format: {Category} | {X} days/week | {Duration} min | {Level}
-- backend filters using LIKE on each token
...
INSERT INTO workout_session (user_id, started_at, ended_at, workout_plan_id, notes) VALUES
  (2,  '2026-03-02 18:00:00', '2026-03-02 18:55:00', 1, 'Push day. Hit all sets.'),              -- 1
  (2,  '2026-03-04 18:00:00', '2026-03-04 19:05:00', 1, 'Pull day. Rows felt heavy.'),           -- 2
  (2,  '2026-03-06 18:00:00', '2026-03-06 19:10:00', 1, 'Legs day. Calf raise PR.'),             -- 3
  (5,  '2026-03-03 07:00:00', '2026-03-03 07:58:00', 3, 'Full body. Fatigued on leg press.'),    -- 4
  (5,  '2026-03-06 07:00:00', '2026-03-06 08:05:00', 3, 'Full body session 2. Felt better.'),    -- 5
  (6,  '2026-03-04 12:00:00', NULL,                   2, 'Upper day. Stopped early.'),           -- 6
  (7,  '2026-03-05 18:00:00', '2026-03-05 19:10:00', 4, 'Arms day. Good pump.'),                 -- 7
  (8,  '2026-03-07 10:00:00', '2026-03-07 11:00:00', 4, 'First time on close grip curl.'),       -- 8
  (9,  '2026-03-03 17:00:00', '2026-03-03 17:50:00', 2, 'Upper day. Kept weights light.'),       -- 9
  (10, '2026-03-04 09:00:00', '2026-03-04 09:45:00', 2, 'First ever gym session. Very proud.');  -- 10

INSERT INTO exercise_set_log (session_id, exercise_id, set_number, reps, weight, rpe, performed_at) VALUES
  (1,  5, 1,  8, 135.00, 7.0, '2026-03-02 18:05:00'),   -- incline bench
  (1,  5, 2,  8, 135.00, 7.5, '2026-03-02 18:08:00'),
  (1,  5, 3,  7, 135.00, 8.5, '2026-03-02 18:11:00'),
  (1, 10, 1, 10, NULL,   6.5, '2026-03-02 18:22:00'),   -- impossible dips
  (1, 10, 2,  9, NULL,   7.5, '2026-03-02 18:25:00'),
  (2, 20, 1, 10,  80.00, 7.0, '2026-03-04 18:05:00'),   -- lever front pulldown
  (2, 21, 1, 10,  90.00, 7.5, '2026-03-04 18:20:00'),   -- lever seated row
  (2, 18, 1, 12,  20.00, 6.5, '2026-03-04 18:35:00'),   -- db reverse spider curl
  (3,  2, 1, 10, 180.00, 8.0, '2026-03-06 18:05:00'),   -- sled leg press
  (3,  1, 1, 12, NULL,   6.0, '2026-03-06 18:25:00'),   -- hack calf raise
  (3,  9, 1,  8,  35.00, 7.5, '2026-03-06 18:40:00'),   -- kb pistol squat
  (4,  2, 1,  8, 180.00, 8.0, '2026-03-03 07:10:00'),   -- sled leg press
  (4,  5, 1,  6, 145.00, 8.5, '2026-03-03 07:25:00'),   -- incline bench
  (4, 28, 1, 12, NULL,   6.0, '2026-03-03 07:45:00'),   -- barbell calf raise
  (5,  2, 1,  8, 185.00, 7.5, '2026-03-06 07:10:00'),   -- sled leg press
  (5,  5, 1,  6, 150.00, 8.0, '2026-03-06 07:25:00'),   -- incline bench
  (6,  5, 1, 10,  95.00, 6.5, '2026-03-04 12:05:00'),   -- incline bench
  (6,  3, 1, 12,  20.00, 6.0, '2026-03-04 12:18:00'),   -- db front raise
  (7,  8, 1, 10,  65.00, 7.5, '2026-03-05 18:05:00'),   -- bb close grip curl
  (7,  7, 1, 12,  25.00, 7.0, '2026-03-05 18:20:00'),   -- db hammer preacher curl
  (7, 11, 1, 10,  55.00, 8.0, '2026-03-05 18:35:00'),   -- bb overhead tri ext
  (8,  8, 1, 10,  55.00, 7.0, '2026-03-07 10:05:00'),   -- bb close grip curl
  (8,  6, 1, 12, NULL,   6.5, '2026-03-07 10:20:00'),   -- cable squatting curl
  (9,  5, 1, 10,  85.00, 6.0, '2026-03-03 17:05:00'),   -- incline bench
  (9, 14, 1, 12,  15.00, 5.5, '2026-03-03 17:20:00'),   -- db one arm upright row
  (10, 5, 1,  8,  65.00, 5.0, '2026-03-04 09:05:00'),   -- incline bench
  (10, 3, 1, 12,  10.00, 5.0, '2026-03-04 09:20:00');   -- db front raise
...
INSERT INTO survey_question (prompt, question_type, active) VALUES
  ('How stressed do you feel today (1-10)?',       'scale',  1),   -- 1
  ('Any notes about your mood?',                   'text',   1),   -- 2
  ('How would you rate your sleep last night?',    'scale',  1),   -- 3
  ('Did you experience any pain during training?', 'yes_no', 1),   -- 4
  ('How motivated do you feel today (1-10)?',      'scale',  1);   -- 5
...
-- points (wallets for all real users, system user excluded)
